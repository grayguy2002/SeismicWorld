...
class GeoModel:
    ...
    def add_vp(self, 
        ...
        self.vp = np.ones(self.Z.shape, dtype=np.float32)
        self.horizon = []

        if fm_list is None: fm_list = [0.3, 0.6]
        if vp_list is None: vp_list = [(3000, 3500), (3500, 5000), (5000, 6500)]
        if h_layer_range is None:
            h_layer_range = [0.1 * (self.Zmax - self.Zmin), 0.2 * (self.Zmax - self.Zmin)]

        fmbt_idx = [int(fm_list[0] * (self.nZ - 1)),
                    int(fm_list[1] * (self.nZ - 1)),
                    int(1.0 * (self.nZ - 1))]

        self.info['vp'] = ['P-wave velocity:\n',
                        f'h_layer_range: {h_layer_range}\n',
                        f'fm_list: {fm_list}\n',
                        f'vp_list: {vp_list}\n',
                        f'vp_disturb: {vp_disturb}\n',
                        f'smooth: {smooth}\n']
        if smooth:
            self.info['vp'].append(f'sigma: {sigma}\n')
        self.info['vp'].append(f'seed: {seed}\n')

        depth_top = self.Zmin
        ind_bottom = 0
        vp_upper = None  # 用 None 标识“上一层还没设定”

        for idx, vp_param in zip(fmbt_idx, vp_list):
            fmbt = self.Zmin + idx * self.dZ
            vp1, vp2 = vp_param
            # 预防 vp_diff 不可达：最大可能差异 = max(|vp2 - candidate|) => 若区间与 vp_upper 不重叠或太窄则需放宽
            while ind_bottom < idx:
                ...
                ind_top = int((depth_top - self.Zmin) // self.dZ)
                ind_bottom = int((depth_bottom - self.Zmin) // self.dZ)

                # ---------- 关键修改开始 ----------
                # 采样新层速度，带最大尝试限制，必要时自适应放宽 vp_diff
                max_attempts = 200
                attempt = 0
                current_vp_diff = vp_diff
                vp = random.uniform(vp1, vp2)

                # 第一层或上一层未知 -> 不做差异限制
                if vp_upper is not None:
                    # 如果 vp_diff 大于当前区间最大可能差异，直接降低
                    max_feasible_diff = max(abs(vp2 - vp1), 1e-6)
                    if current_vp_diff > max_feasible_diff:
                        current_vp_diff = 0.6 * max_feasible_diff  # 自适应下调

                    while abs(vp - vp_upper) < current_vp_diff:
                        attempt += 1
                        if attempt >= max_attempts:
                            # 进一步放宽要求
                            current_vp_diff *= 0.7
                            attempt = 0
                            if current_vp_diff < 1:  # 差异过小就放弃约束
                                break
                        vp = random.uniform(vp1, vp2)
                # ---------- 关键修改结束 ----------
                ...
                if vp_disturb > 0.0:
                    ...
                    self.vp[:, :, ind_top:ind_bottom + 1] = np.random.normal(
                        loc=vp, scale=vp_disturb, size=shape
                    )
                else:
                    self.vp[:, :, ind_top:ind_bottom + 1] *= vp

                hz = self.Zmin + (ind_bottom + 1) * self.dZ
                self.horizon.append(Horizon(z=hz, vp=vp, rgt=ind_bottom + 1, channel=0))
                depth_top = hz
    ...
    def compute_gravity(self,
                        ...
                        density_contrast: np.ndarray = None,
                        G: float = 6.67430e-11,
                        mute: bool = False):
        """
        基于 3D 频域公式的重力垂向异常计算 (g_z)，适用于任意 3D 密度对比体。
        gz(k) = 4πG (i kz / k^2) ρ(k)
        
        参数:
            height: 观测面高出模型顶部的高度 (m)。>0 时采用简单向上延拓 e^{-k*height}。
            padding_factor: XY 方向零填充倍数 (≥1) 降低频谱混叠。
            density_contrast: 若提供则使用 (kg/m^3)，否则由 self.rho - rho_ref 生成。
                            self.rho 当前为 g/cm^3 -> 自动转换 kg/m^3。
            G: 万有引力常数。
            mute: 静默输出。
        返回:
            gravity_map: 2D numpy array (ny, nx)，单位 mGal。
        """
        if self.rho is None and density_contrast is None:
            raise ValueError("缺少密度模型。请先调用 add_density_model 或传入 density_contrast。")

        if not mute:
            print("[Gravity] 开始计算 3D 频域重力异常...")

        # 1. 准备密度对比 (kg/m^3)
        if density_contrast is None:
            # 将 g/cm^3 转换为 kg/m^3  (1 g/cm^3 = 1000 kg/m^3)
            rho_si = (self.rho.astype(np.float64) - 2.67) * 1000.0  # 采用 2.67 g/cm^3 作为参考
        else:
            rho_si = density_contrast.astype(np.float64)

        ny, nx, nz = rho_si.shape

        # 2. 零填充 (仅 XY)
        py = ny * (padding_factor - 1)
        px = nx * (padding_factor - 1)
        if padding_factor > 1:
            rho_pad = np.pad(rho_si, ((0, py), (0, px), (0, 0)), mode="constant")
        else:
            rho_pad = rho_si
        ny_p, nx_p, nz_p = rho_pad.shape

        dy, dx, dz = self.dY, self.dX, self.dZ

        # 3. 3D FFT
        rho_k = np.fft.fftn(rho_pad, axes=(0,1,2))

        ky = 2*np.pi * np.fft.fftfreq(ny_p, d=dy)
        kx = 2*np.pi * np.fft.fftfreq(nx_p, d=dx)
        kz = 2*np.pi * np.fft.fftfreq(nz_p, d=dz)
        ...
        # 4. 频域公式 gz(k)
        #   gz(k) = 4πG (i kz / k^2) * ρ(k)
        gz_k = 4*np.pi*G * (1j * Kz / k2) * rho_k

        # 5. 向上延拓（观测高度）
        if height > 0:
            k_h = np.sqrt(Kx**2 + Ky**2 + Kz**2)  # 严格 3D 延拓
            gz_k *= np.exp(-k_h * height)

        # 6. IFFT
        gz_vol = np.fft.ifftn(gz_k, axes=(0,1,2)).real  # m/s^2
        ...
        # 8. 去除填充
        if padding_factor > 1:
            gz_surface = gz_surface[:ny, :nx]

        # 9. 转 mGal
        gravity_map = gz_surface * 1e5

        if not mute:
            print(f"[Gravity] 结果范围: {gravity_map.min():.3f} ~ {gravity_map.max():.3f} mGal")

        self.gravity = gravity_map
        return gravity_map
    ...
    def compute_magnetic_anomaly(self,
                                ...
                                mute: bool = False):
        """
        频域分层积分 (2D FFT + 深度叠加) 计算总磁强度 (TMI) 异常。
        ΔT(k) = F_hat · B(k)
        B(k) = μ0 (I - kk^T/k^2) Σ_z [ M(k,z) e^{-k(h+z)} Δz ]
        
        参数:
            height: 观测高度 (m) 相对模型顶部 (z=0)。
            inclination: 磁倾角 (deg，正向下)。
            declination: 磁偏角 (deg，正向东)。
            intensity: 总场强度 (nT)。
            padding_factor: XY 零填充倍数。
            mute: 静默。
        返回:
            anomaly_map: 2D numpy array, 单位 nT
        """
        if self.magnetic_susceptibility is None:
            raise ValueError("缺少磁化率模型，请先调用 add_magnetic_susceptibility。")

        if not mute:
            print("[Mag] 开始 2D 分层磁异常计算...")

        mu0 = 4*np.pi*1e-7
        F_T = intensity * 1e-9  # nT -> T
        # H = F / μ0
        H = F_T / mu0
        ...
        Fz = np.sin(inc)
        F_hat = np.array([Fx, Fy, Fz], dtype=np.float64)

        κ = self.magnetic_susceptibility.astype(np.float64)
        ny, nx, nz = κ.shape
        dy, dx, dz = self.dY, self.dX, self.dZ
        ...
        if padding_factor > 1:
            pad_cfg = ((0,py),(0,px))
            def pad2(a):
                return np.pad(a, pad_cfg, mode="constant")
        else:
            def pad2(a): return a

        ny_p, nx_p = ny+py, nx+px
        ky = 2*np.pi * np.fft.fftfreq(ny_p, d=dy)
        kx = 2*np.pi * np.fft.fftfreq(nx_p, d=dx)
        ...
        # 3. 深度积分的频谱累加
        Sx = np.zeros((ny_p, nx_p), dtype=np.complex128)
        Sy = np.zeros_like(Sx)
        Sz = np.zeros_like(Sx)

        z_coords = np.arange(nz)*dz  # z=0 为顶部，正向下
        for iz, zval in enumerate(z_coords):
            mx2 = pad2(Mx[:,:,iz])
            my2 = pad2(My[:,:,iz])
            mz2 = pad2(Mz[:,:,iz])
            Mxk = np.fft.fft2(mx2)
            Myk = np.fft.fft2(my2)
            Mzk = np.fft.fft2(mz2)
            decay = np.exp(-k_h*(height + zval))
            Sx += Mxk * decay * dz
            Sy += Myk * decay * dz
            Sz += Mzk * decay * dz

        # 4. 构造张量投影 (I - kk^T/k^2) 这里 k_z=0（仅水平波数，垂向通过层积分）
        # 视作 2D 水平场延拓的有效核
        kx_norm = Kx / k_h
        ky_norm = Ky / k_h

        # (I - kk^T/k^2) 对 (Sx, Sy, Sz)
        # 水平部分
        Bx_k = mu0 * ( (1 - kx_norm*kx_norm)*Sx - kx_norm*ky_norm*Sy )
        By_k = mu0 * ( -ky_norm*kx_norm*Sx + (1 - ky_norm*ky_norm)*Sy )
        # 垂向分量： (1 - 0) * Sz - (k_z=0 相关项)= mu0 * Sz
        Bz_k = mu0 * Sz
        ...
        # 6. 投影到总场方向
        deltaT_T = Bx*Fx + By*Fy + Bz*Fz  # Tesla
        anomaly_full = deltaT_T * 1e9     # -> nT

        # 7. 去填充
        anomaly_map = anomaly_full[:ny, :nx]

        if not mute:
            print(f"[Mag] TMI 范围: {anomaly_map.min():.2f} ~ {anomaly_map.max():.2f} nT")

        self.magnetic_anomaly = anomaly_map
        return anomaly_map

    def compute_magnetic_anomaly_3d(self,
                                    height: float = 0.0,
                                    ...
                                    mute: bool = False):
        """
        3D FFT 磁异常（简化版本），假定周期延拓。对全体磁化体做 3D 卷积。
        B(k) = μ0 (I - kk^T/k^2) M(k)
        ΔT = F_hat · B
        观测高度 height>0 时，近似采用对 2D 水平频谱的 e^{-k_h * height} 上延拓 (在取顶部切片后)。

        返回:
            anomaly_map (ny, nx) nT
        """
        if self.magnetic_susceptibility is None:
            raise ValueError("缺少磁化率模型。")

        if not mute:
            print("[Mag-3D] 开始 3D FFT 磁异常计算...")
        ...
        py = ny*(padding_factor-1)
        px = nx*(padding_factor-1)
        pz = nz*(padding_factor-1)
        if padding_factor > 1:
            pad_cfg = ((0,py),(0,px),(0,pz))
            Mx = np.pad(Mx, pad_cfg, mode="constant")
            My = np.pad(My, pad_cfg, mode="constant")
            Mz = np.pad(Mz, pad_cfg, mode="constant")
        ny_p, nx_p, nz_p = Mx.shape
        ...
        # 3. B(k)
        # 投影算子 P_ij = δ_ij - k_i k_j / k^2
        kx_n = Kx / np.sqrt(k2)
        ky_n = Ky / np.sqrt(k2)
        kz_n = Kz / np.sqrt(k2)

        # 先计算 dot(k̂, M_k)
        k_dot_M = kx_n*Mx_k + ky_n*My_k + kz_n*Mz_k
        Bx_k = mu0 * (Mx_k - kx_n * k_dot_M)
        By_k = mu0 * (My_k - ky_n * k_dot_M)
        Bz_k = mu0 * (Mz_k - kz_n * k_dot_M)
        ...
        # 5. 取顶部切片 (z=0)，如需 height>0，上延拓 2D 频谱
        Bx0 = Bx[:,:,0]
        By0 = By[:,:,0]
        Bz0 = Bz[:,:,0]
        ...
        if height > 0:
            # 2D FFT 水平延拓
            Bx0_k = np.fft.fft2(Bx0)
            By0_k = np.fft.fft2(By0)
            Bz0_k = np.fft.fft2(Bz0)

            ky2 = 2*np.pi * np.fft.fftfreq(ny_p, d=dy)
            kx2 = 2*np.pi * np.fft.fftfreq(nx_p, d=dx)
            Ky2, Kx2 = np.meshgrid(ky2, kx2, indexing='ij')
            k_h = np.sqrt(Kx2**2 + Ky2**2)
            decay = np.exp(-k_h * height)

            Bx0 = np.fft.ifft2(Bx0_k * decay).real
            By0 = np.fft.ifft2(By0_k * decay).real
            Bz0 = np.fft.ifft2(Bz0_k * decay).real

        # 6. 投影到 F_hat
        deltaT_T = Bx0*Fx + By0*Fy + Bz0*Fz
        anomaly_full = deltaT_T * 1e9  # nT
        ...
        if not mute:
            print(f"[Mag-3D] TMI 范围: {anomaly_map.min():.2f} ~ {anomaly_map.max():.2f} nT")
    ...
    def simulate_co2_storage_timelapse_fast(
            self,
            injection_rates,
            co2_saturation_effect=0.3,
            pressure_buildup_coefficient=0.1,
            co2_density_contrast=-0.3,
            tuning_thickness_factor=2.5,
            noise_level=0.02,
            compute_potential='final',  # 'none'|'baseline_only'|'final'
            wavelet_type='ricker',
            f_ricker=30,
            dt=0.002,
            seed=None):
        ...
        if self.vp is None: raise ValueError("Need vp")
        if self.rc is None: self.compute_rc(mute=True)
        self.make_synseis(wavelet_type=wavelet_type, f_ricker=f_ricker, dt=dt, mute=True)

        # 基线
        out = []
        vp0 = self.vp.copy()
        rc0 = self.rc.copy()
        rho_base = getattr(self,'rho', None)
        kappa_base = getattr(self,'magnetic_susceptibility', None)

        out.append(dict(time_step=0,
                        injection_rate=0.0,
                        vp=vp0.copy(),
                        rc=rc0.copy(),
                        seismic=self.seismic.copy()))
        ...
        # 预计算掩码
        if self.channel is not None:
            inj_mask = (self.channel > 0).astype(np.float32)
        else:
            z_mid = self.vp.shape[2]//2
            inj_mask = np.zeros_like(self.vp, dtype=np.float32)
            inj_mask[:,:,z_mid-5:z_mid+5] = 1.0

        ny,nx,nz = self.vp.shape
        z = np.arange(nz, dtype=np.float32)*self.dZ
        z_norm = z / z.max()

        for step, rate in enumerate(injection_rates):
            # 饱和度分布（简单：掩码 * rate * 垂向衰减）
            sat = inj_mask * rate * np.exp(-2.0*z_norm)[None,None,:]

            # 压力变化
            pres = sat * pressure_buildup_coefficient

            # 速度变化（向量化）
            dv = (-co2_saturation_effect * sat + 0.5*co2_saturation_effect*pres) * vp0
            vp_new = vp0 + dv

            # RC + 合成
            self.vp = vp_new
            self.compute_rc(mute=True, time_lapse=True)
            # 调谐：对 rc 在 sat>阈值区域进行 1D 平滑
            from scipy.ndimage import gaussian_filter1d
            rc_tune = self.rc.copy()
            sat_mask = (sat > 0.05)
            # 对每个 (y,x) 仅一次 1D
            kern_sigma = tuning_thickness_factor
            for iy in range(ny):
                for ix in range(nx):
                    if sat_mask[iy,ix].any():
                        rc_tune[iy,ix] = gaussian_filter1d(rc_tune[iy,ix], sigma=kern_sigma)

            self.rc = 0.7*self.rc + 0.3*rc_tune
            self.make_synseis(wavelet_type=wavelet_type, f_ricker=f_ricker, dt=dt, mute=True)

            if noise_level>0:
                self.seismic += np.random.normal(0, noise_level*np.std(self.seismic), self.seismic.shape).astype(np.float32)

            # 可选：只最终步计算重力/磁
            gravity=None; magnetic=None
            if compute_potential=='final' and step == len(injection_rates)-1:
                if rho_base is not None:
                    self.rho = rho_base + sat * co2_density_contrast
                    self.compute_gravity(height=50.0, padding_factor=1, mute=True)
                    gravity = self.gravity.copy()
                if kappa_base is not None:
                    self.magnetic_susceptibility = kappa_base * (1 + 0.1*sat)
                    self.compute_magnetic_anomaly_3d(height=10.0, padding_factor=1, mute=True)
                    magnetic = self.magnetic_anomaly.copy()
